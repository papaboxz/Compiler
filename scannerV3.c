...
Token malar_next_token(void)
{
	...
	//Used to validate the value from b_getcoffset() calls
	short tempOffset;
...
Token aa_func05(char lexeme[]){
	...
	if (atoi(lexeme) <= (pow(2, 15) - 1) && atoi(lexeme) >= -pow(2, 15) && strlen(lexeme) <= INL_LEN)
	{
...
Token aa_func08(char lexeme[]) {
	...
	if (atof(lexeme) < (pow(2, 31) - 1) && atof(lexeme) > -(pow(2, 31)))
	{
